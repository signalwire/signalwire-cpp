...
int main() {
  ...
  auto search =
      datamap::DataMap("multi_search")
          ...
          .parameter("priority", "string", "fast or comprehensive", false,
                     {"fast", "comprehensive"})
          .webhook("GET", "https://api.fast.com/q?term=${args.query}", {{"X-API-Key", "FAST_KEY"}})
          .webhook("GET", "https://api.fallback.com/search?q=${args.query}",
                   {{"Authorization", "Bearer TOKEN"}})
          .foreach (
              {{"input_key", "${response.items}"}, {"output_key", "foreach"}, {"append", true}})
  ...
  auto router =
      datamap::DataMap("route_request")
          .description("Route requests by type")
          .parameter("type", "string", "Request type", true)
          .expression(
              "${args.type}", "billing.*",
              swaig::FunctionResult("Routing to billing department").connect("+15551001", true))
          .expression("${args.type}", "tech.*",
                      swaig::FunctionResult("Routing to tech support").connect("+15551002", true))
          .expression("${args.type}", "sales.*",
                      swaig::FunctionResult("Routing to sales").connect("+15551003", true));
