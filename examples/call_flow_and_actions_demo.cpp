...
int main() {
  ...
  try {
    ...
    agent.add_post_answer_verb("record_call", {{"stereo", true}, {"format", "wav"}});
    ...
    agent.define_tool(
        ...
        {{"type", "object"},
         {"properties", {{"number", {{"type", "string"}, {"description", "Phone number"}}}}}},
        [](const json& args, const json& raw) -> swaig::FunctionResult {
          (void)raw;
          std::string num = args.value("number", "");
          return swaig::FunctionResult("Transferring to " + num).connect(num, true);
        });
