  ...
  auto file_ctrl =
      datamap::DataMap("file_control")
          ...
          .expression("${args.command}", "stop.*",
                      swaig::FunctionResult("Stopping playback").add_action("stop_playback", true));
  agent.register_swaig_function(file_ctrl.to_swaig_function());

  // 3. Knowledge search with foreach
  auto search =
      datamap::DataMap("search_knowledge")
          ...
          .foreach (
              {{"input_key", "${response.results}"}, {"output_key", "foreach"}, {"append", true}})
