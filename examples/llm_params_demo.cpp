...
int main() {
  ...
  try {
    ...
    agent.set_post_prompt_llm_params({{"temperature", 0.1}, {"top_p", 0.95}});

    agent.set_post_prompt(
        "Classify the issue: {\"category\": \"...\", \"severity\": \"low|medium|high\"}");

    agent.set_params(
        {{"ai_model", "gpt-4.1-nano"}, {"end_of_speech_timeout", 1500}, {"ai_volume", 5}});
