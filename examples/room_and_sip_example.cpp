...
int main() {
  ...
  try {
    ...
    agent.define_tool(
        "join_meeting", "Join a video room",
        {{"type", "object"},
        ...
        [](const json& args, const json& raw) -> swaig::FunctionResult {
          ...
          return swaig::FunctionResult("Joining room: " + room).join_room(room);
        });

    agent.define_tool(
        "sip_transfer", "Transfer via SIP REFER",
        {{"type", "object"},
         {"properties", {{"uri", {{"type", "string"}, {"description", "SIP URI"}}}}}},
        [](const json& args, const json& raw) -> swaig::FunctionResult {
          (void)raw;
          std::string uri = args.value("uri", "");
          return swaig::FunctionResult("Transferring via SIP to " + uri).sip_refer(uri);
        });

    agent.define_tool(
        "start_conference", "Start a conference call",
        {{"type", "object"},
         {"properties", {{"name", {{"type", "string"}, {"description", "Conference name"}}}}}},
        [](const json& args, const json& raw) -> swaig::FunctionResult {
          (void)raw;
