...
int main() {
  ...
  agent.prompt_add_section(
      "Instructions", "",
      {"Track conversation topics in global data", "Use session tokens for secure tool calls"});

  agent.set_global_data({{"session_type", "demo"}, {"interaction_count", 0}});
  ...
  agent.define_tool(
      "update_topic", "Track the current topic",
      {{"type", "object"},
       {"properties", {{"topic", {{"type", "string"}, {"description", "Current topic"}}}}}},
