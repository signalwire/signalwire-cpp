...
int main() {
  ...
  try {
    ...
    a.set_dynamic_config_callback([](const std::map<std::string, std::string>& qp, const json& body,
      ...
      // AI parameters
      ephemeral.set_params({{"end_of_speech_timeout", is_vip ? 300 : 500},
      ...
      if (department == "sales") {
        ephemeral.prompt_add_section("Sales Expertise", "You specialize in sales:",
                                     {"Present product features and benefits",
                                      "Handle pricing questions", "Process orders and upgrades"});
        ...
        ephemeral.prompt_add_section("Billing Expertise", "You specialize in billing:",
                                     {"Explain statements and charges",
                                      "Process payment arrangements", "Handle dispute resolution"});
        ephemeral.add_hints({"invoice", "payment", "charges"});
      } else {
        ephemeral.prompt_add_section(
            "Support Guidelines", "Follow these principles:",
      ...
      ephemeral.define_tool(
          ...
          [](const json& args, const json& raw) -> swaig::FunctionResult {
            ...
            return swaig::FunctionResult("Order " + num +
                                         " is being processed. Ships in 2 business days.");
          });
    });
