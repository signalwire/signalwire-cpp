...
int main() {
  ...
  try {
    ...
    agent.define_tool(
        "manage_state", "Update conversation state",
        {{"type", "object"},
         {"properties",
    ...
    agent.define_tool(
        ...
        {{"type", "object"},
         {"properties", {{"url", {{"type", "string"}, {"description", "Audio URL"}}}}}},
        [](const json& args, const json& raw) -> swaig::FunctionResult {
          (void)raw;
    ...
    agent.define_tool(
        "switch", "Switch conversation context",
        {{"type", "object"},
         {"properties", {{"prompt", {{"type", "string"}, {"description", "New system prompt"}}}}}},
        [](const json& args, const json& raw) -> swaig::FunctionResult {
          (void)raw;
    ...
    agent.define_tool("send_text", "Send an SMS",
                      ...
                      [](const json& args, const json& raw) -> swaig::FunctionResult {
                        ...
                        return swaig::FunctionResult("Text sent")
                            .send_sms(args.value("to", ""), "+15559876543",
                                      args.value("message", ""));
                      });
