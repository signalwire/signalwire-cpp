...
int main() {
  ...
  if (!api_key || !engine_id || std::string(api_key).empty() || std::string(engine_id).empty()) {
    ...
    agent.add_skill("web_search",
                    {{"api_key", api_key}, {"search_engine_id", engine_id}, {"num_results", 3}});

    // News search
    agent.add_skill("web_search", {{"api_key", api_key},
