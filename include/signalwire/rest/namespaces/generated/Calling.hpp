...
class Calling {
 public:
  ...
  [[nodiscard]] json dial(const DialParams& p) const {
      ...
      params["from"] = p.from;
      params["to"] = p.to;
      if (p.caller_id.has_value()) { params["caller_id"] = *p.caller_id; }
      if (p.fallback_url.has_value()) { params["fallback_url"] = *p.fallback_url; }
      if (p.status_url.has_value()) { params["status_url"] = *p.status_url; }
      if (p.status_events.has_value()) { params["status_events"] = *p.status_events; }
      if (p.url_method.has_value()) { params["url_method"] = *p.url_method; }
      if (p.url.has_value()) { params["url"] = *p.url; }
      if (p.codecs.has_value()) { params["codecs"] = *p.codecs; }
      if (p.swml.has_value()) { params["swml"] = *p.swml; }
      if (!p.extras.is_null()) { params.update(p.extras); }
  ...
  [[nodiscard]] json update(const UpdateParams& p) const {
      ...
      params["id"] = p.id;
      if (p.fallback_url.has_value()) { params["fallback_url"] = *p.fallback_url; }
      if (p.status.has_value()) { params["status"] = *p.status; }
      if (p.status_url.has_value()) { params["status_url"] = *p.status_url; }
      if (p.url.has_value()) { params["url"] = *p.url; }
      if (p.swml.has_value()) { params["swml"] = *p.swml; }
      if (!p.extras.is_null()) { params.update(p.extras); }
  ...
  [[nodiscard]] json end(const std::string& call_id, const EndParams& p) const {
      json params = json::object();
      if (p.reason.has_value()) { params["reason"] = *p.reason; }
      if (!p.extras.is_null()) { params.update(p.extras); }
    return execute("calling.end", params, call_id);
  }
  [[nodiscard]] json ai_hold(const std::string& call_id, const Ai_holdParams& p) const {
      json params = json::object();
      if (p.timeout.has_value()) { params["timeout"] = *p.timeout; }
      if (p.prompt.has_value()) { params["prompt"] = *p.prompt; }
      if (!p.extras.is_null()) { params.update(p.extras); }
  ...
  [[nodiscard]] json ai_message(const std::string& call_id, const Ai_messageParams& p) const {
      json params = json::object();
      if (p.role.has_value()) { params["role"] = *p.role; }
      if (p.message_text.has_value()) { params["message_text"] = *p.message_text; }
      if (p.reset.has_value()) { params["reset"] = *p.reset; }
      if (p.global_data.has_value()) { params["global_data"] = *p.global_data; }
      if (!p.extras.is_null()) { params.update(p.extras); }
    return execute("calling.ai_message", params, call_id);
  }
  [[nodiscard]] json live_transcribe(const std::string& call_id, const Live_transcribeParams& p) const {
      json params = json::object();
      params["action"] = p.action;
      if (!p.extras.is_null()) { params.update(p.extras); }
    return execute("calling.live_transcribe", params, call_id);
  }
  [[nodiscard]] json live_translate(const std::string& call_id, const Live_translateParams& p) const {
      json params = json::object();
      params["action"] = p.action;
      if (p.status_url.has_value()) { params["status_url"] = *p.status_url; }
      if (!p.extras.is_null()) { params.update(p.extras); }
  ...
  [[nodiscard]] json play(const std::string& call_id, const PlayParams& p) const {
      json params = json::object();
      params["play"] = p.play;
      if (p.control_id.has_value()) { params["control_id"] = *p.control_id; }
      if (p.volume.has_value()) { params["volume"] = *p.volume; }
      if (p.direction.has_value()) { params["direction"] = *p.direction; }
      if (p.loop.has_value()) { params["loop"] = *p.loop; }
      if (p.status_url.has_value()) { params["status_url"] = *p.status_url; }
      if (!p.extras.is_null()) { params.update(p.extras); }
  ...
  [[nodiscard]] json record(const std::string& call_id, const RecordParams& p) const {
      json params = json::object();
      if (p.control_id.has_value()) { params["control_id"] = *p.control_id; }
      if (p.audio.has_value()) { params["audio"] = *p.audio; }
      if (p.status_url.has_value()) { params["status_url"] = *p.status_url; }
      if (!p.extras.is_null()) { params.update(p.extras); }
  ...
  [[nodiscard]] json collect(const std::string& call_id, const CollectParams& p) const {
      json params = json::object();
      if (p.control_id.has_value()) { params["control_id"] = *p.control_id; }
      if (p.initial_timeout.has_value()) { params["initial_timeout"] = *p.initial_timeout; }
      if (p.digits.has_value()) { params["digits"] = *p.digits; }
      if (p.speech.has_value()) { params["speech"] = *p.speech; }
      if (p.continuous.has_value()) { params["continuous"] = *p.continuous; }
      if (p.partial_results.has_value()) { params["partial_results"] = *p.partial_results; }
      if (!p.extras.is_null()) { params.update(p.extras); }
  ...
  [[nodiscard]] json collect_start_input_timers(const std::string& call_id, const Collect_start_input_timersParams& p) const {
      json params = json::object();
      params["control_id"] = p.control_id;
      if (!p.extras.is_null()) { params.update(p.extras); }
  ...
  [[nodiscard]] json stream(const std::string& call_id, const StreamParams& p) const {
      ...
      params["url"] = p.url;
      if (p.control_id.has_value()) { params["control_id"] = *p.control_id; }
      if (p.codec.has_value()) { params["codec"] = *p.codec; }
      if (p.track.has_value()) { params["track"] = *p.track; }
      if (p.authorization_bearer_token.has_value()) { params["authorization_bearer_token"] = *p.authorization_bearer_token; }
      if (p.custom_parameters.has_value()) { params["custom_parameters"] = *p.custom_parameters; }
      if (!p.extras.is_null()) { params.update(p.extras); }
  ...
  [[nodiscard]] json transcribe_stop(const std::string& call_id, const Transcribe_stopParams& p) const {
      json params = json::object();
      params["control_id"] = p.control_id;
      if (!p.extras.is_null()) { params.update(p.extras); }
  ...
  [[nodiscard]] json receive_fax_stop(const std::string& call_id, const Receive_fax_stopParams& p) const {
      json params = json::object();
      params["control_id"] = p.control_id;
      if (!p.extras.is_null()) { params.update(p.extras); }
 ...
 private:
  [[nodiscard]] json execute(const std::string& command, const json& params,
    ...
    if (call_id.has_value()) { body["id"] = *call_id; }
