...
class DatasphereDocuments : public CrudResource {
 public:
  ...
  [[nodiscard]] json search(const SearchParams& p) const {
      ...
      if (p.tags.has_value()) { body["tags"] = *p.tags; }
      if (p.document_id.has_value()) { body["document_id"] = *p.document_id; }
      if (p.distance.has_value()) { body["distance"] = *p.distance; }
      if (p.count.has_value()) { body["count"] = *p.count; }
      if (p.language.has_value()) { body["language"] = *p.language; }
      if (p.pos_to_expand.has_value()) { body["pos_to_expand"] = *p.pos_to_expand; }
      if (p.max_synonyms.has_value()) { body["max_synonyms"] = *p.max_synonyms; }
      if (!p.extras.is_null()) { body.update(p.extras); }
  return client_.post(base_path_ + "/" + std::string("search"), body);
  }

  [[nodiscard]] json listChunks(const std::string& documentId, const std::map<std::string, std::string>& params = {}) const {
  return client_.get(base_path_ + "/" + documentId + "/" + std::string("chunks"), params);
  }

  [[nodiscard]] json getChunk(const std::string& documentId, const std::string& chunkId, const std::map<std::string, std::string>& params = {}) const {
  return client_.get(base_path_ + "/" + documentId + "/" + std::string("chunks") + "/" + chunkId, params);
  }
