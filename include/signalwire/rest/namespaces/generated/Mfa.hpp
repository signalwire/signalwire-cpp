...
class Mfa : public BaseResource {
 public:
  ...
  explicit Mfa(const HttpClient& client)
      : BaseResource(client, "/api/relay/rest/mfa") {}

  [[nodiscard]] json sms(const SmsParams& p) const {
      json body = json::object();
      body["to"] = p.to;
      if (p.from.has_value()) { body["from"] = *p.from; }
      if (p.message.has_value()) { body["message"] = *p.message; }
      if (p.token_length.has_value()) { body["token_length"] = *p.token_length; }
      if (p.valid_for.has_value()) { body["valid_for"] = *p.valid_for; }
      if (p.max_attempts.has_value()) { body["max_attempts"] = *p.max_attempts; }
      if (p.allow_alphas.has_value()) { body["allow_alphas"] = *p.allow_alphas; }
