// Copyright (c) 2025 SignalWire
// SPDX-License-Identifier: MIT
//
// Code generated by scripts/generate_rest.py; DO NOT EDIT.
//
// Generated REST wire type for the 'video' namespace (components/schemas 'RoomSessionSummary').
#pragma once

#include <map>
#include <nlohmann/json.hpp>
#include <optional>
#include <string>
#include <vector>

namespace signalwire {
namespace rest {
namespace generated {
namespace types {
namespace Video {

using json = nlohmann::json;

/// RoomSessionSummary — generated read-side data type.
/// Generated REST wire type for the 'video' namespace (components/schemas 'RoomSessionSummary').
///
/// Method-less DTO: one typed member per snake wire key + open `extras`.
struct RoomSessionSummary {
  std::optional<std::string> id;
  std::optional<json> room_id;
  std::optional<json> name;
  std::optional<json> display_name;
  std::optional<json> max_members;
  std::optional<json> quality;
  std::optional<json> fps;
  std::optional<json> join_from;
  std::optional<json> join_until;
  std::optional<json> remove_at;
  std::optional<json> remove_after_seconds_elapsed;
  std::optional<json> layout;
  std::optional<bool> record_on_start;
  std::optional<bool> tone_on_entry_and_exit;
  std::optional<bool> room_join_video_off;
  std::optional<bool> user_join_video_off;
  std::optional<bool> locked;
  std::optional<json> start_time;
  std::optional<json> end_time;
  std::optional<json> duration;
  std::optional<json> status;
  std::optional<std::string> created_at;
  std::optional<std::string> updated_at;
  std::optional<json> preview_url;
  std::optional<json> prioritize_handraise;
  std::optional<json> sync_audio_video;
  json extras = json::object();
};

}  // namespace Video
}  // namespace types
}  // namespace generated
}  // namespace rest
}  // namespace signalwire
