...
[[nodiscard]] inline std::vector<std::string> record_format_values() {
    return {record_format_value(RecordFormat::Wav),
            record_format_value(RecordFormat::Mp3),
...
[[nodiscard]] inline std::vector<std::string> tap_direction_values() {
    return {tap_direction_value(TapDirection::Speak),
            tap_direction_value(TapDirection::Hear),
            tap_direction_value(TapDirection::Both)};
}
[[nodiscard]] inline std::vector<std::string> codec_values() {
    return {codec_value(Codec::Pcmu),
            codec_value(Codec::Pcma)};
}
...
public:
    ...
    ParameterSchema& enum_of(const std::string& name,
    ...
    /// "items":{"type":"string"}}`).
    ParameterSchema& array_of(const std::string& name,
    ...
    ParameterSchema& object_of(const std::string& name,
...
private:
    ParameterSchema& add_scalar(const std::string& name,
