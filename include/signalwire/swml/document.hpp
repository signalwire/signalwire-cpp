...
struct Verb {
  ...
  [[nodiscard]] json to_json() const { return json::object({{name, params}}); }
};
...
struct Section {
  ...
  void add_verb(const Verb& verb) { verbs.push_back(verb); }
...
class Document {
 public:
  Document() : version_("1.0.0") { sections_["main"] = Section("main"); }
  ...
  Section& main() { return section("main"); }
  ...
  Document& add_verb_to_section(const std::string& section_name, const std::string& verb_name,
                                const json& params) {
  ...
  [[nodiscard]] std::string to_string(int indent = -1) const { return to_json().dump(indent); }
