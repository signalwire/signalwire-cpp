...
class SchemaValidationError : public std::runtime_error {
 ...
 private:
  static std::string build_message(const std::string& vn, const std::vector<std::string>& errs);
...
class SchemaUtils {
 ...
 private:
  ...
  std::pair<bool, std::vector<std::string>> validate_verb_full(const std::string& verb_name,
                                                               const json& verb_config) const;
  ...
  /// object-with-properties (so no shallow key check applies).
