...
#
# Assumes the build/ tree has already been configured via `cmake -S . -B build`.
# If it hasn't, the TEST gate will fail with a clear message — re-run cmake
# bootstrap and re-run this script.
...
# Bootstrap build/ if missing (configure step is part of CI bring-up).
if [ ! -d build ]; then
    echo "[BOOTSTRAP] cmake -S . -B build (initial configure)"
    cmake -S . -B build || {
        echo "[BOOTSTRAP] FAIL: cmake configure failed"
        exit 1
    }
fi

# Gate 1: build + run tests
run_gate "TEST" "cmake build run_tests + ./build/run_tests" \
    bash -c "cmake --build build --target run_tests -j && ./build/run_tests"
