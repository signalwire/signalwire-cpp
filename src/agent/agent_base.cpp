...
AgentBase& AgentBase::prompt_add_section(const std::string& title, const std::string& body,
...
AgentBase& AgentBase::prompt_add_to_section(const std::string& title, const std::string& body,
                                            const std::vector<std::string>& bullets) {
  for (auto& section : pom_sections_) {
...
std::optional<std::string> AgentBase::get_post_prompt() const { return post_prompt_text_; }

std::optional<std::string> AgentBase::get_raw_prompt() const { return raw_prompt_text_; }
...
AgentBase& AgentBase::define_tool(const std::string& name, const std::string& description,
                                  const json& parameters, swaig::ToolHandler handler, bool secure) {
...
swaig::FunctionResult AgentBase::on_function_call(const std::string& name, const json& args,
                                                  const json& raw_data) {
...
bool AgentBase::validate_tool_token(const std::string& function_name, const std::string& token,
                                    const std::string& call_id) const {
...
AgentBase& AgentBase::add_pronunciation(const std::string& replace_val, const std::string& with_val,
                                        bool ignore_case) {
...
AgentBase& AgentBase::set_internal_fillers(const json& fillers) {
  if (fillers.is_object()) {
    ...
    if (!unknown.empty()) {
      ...
      unknown_str += "]";
      get_logger().warn("unknown_internal_filler_names: " + unknown_str +
...
AgentBase& AgentBase::add_internal_filler(const std::string& lang,
  ...
  if (internal_fillers_.is_null()) {
    internal_fillers_ = json::object();
  }
...
AgentBase& AgentBase::add_internal_filler(const std::string& function_name,
  ...
  if (supported.count(function_name) == 0) {
    get_logger().warn("unknown_internal_filler_name: '" + function_name +
...
AgentBase& AgentBase::clear_pre_answer_verbs() {
  pre_answer_verbs_.clear();
  return *this;
}
AgentBase& AgentBase::clear_post_answer_verbs() {
  post_answer_verbs_.clear();
  return *this;
}
AgentBase& AgentBase::clear_post_ai_verbs() {
  post_ai_verbs_.clear();
  return *this;
}
...
AgentBase& AgentBase::add_skill(const std::string& skill_name, const json& params) {
  ...
  for (const auto& section : skill->get_prompt_sections()) {
    ...
    for (const auto& b : section.bullets) {
      bullets.push_back(b);
    }
...
bool AgentBase::has_skill(const std::string& skill_name) const {
  return std::find(loaded_skills_.begin(), loaded_skills_.end(), skill_name) !=
         loaded_skills_.end();
}
...
std::vector<std::string> AgentBase::list_skills() const { return loaded_skills_; }
...
AgentBase& AgentBase::add_mcp_server(const std::string& url,
  ...
  if (!resource_vars.empty()) {
    json rv = json::object();
    for (const auto& [k, v] : resource_vars) {
      rv[k] = v;
    }
...
std::vector<json> AgentBase::build_mcp_tool_list() const {
  ...
  for (const auto& name : tool_order_) {
    ...
    if (it != tools_.end()) {
      ...
      tool["description"] =
          it->second.description.empty() ? it->second.name : it->second.description;
...
json AgentBase::handle_mcp_request(const json& body) {
  ...
  if (method == "tools/call") {
    ...
    try {
      json raw_data =
          json::object({{"function", tool_name},
      ...
      return json::object(
          {{"jsonrpc", "2.0"},
           {"id", req_id},
           {"result", json::object({{"content", json::array({json::object(
...
std::optional<std::string> AgentBase::signing_key() const { return signing_key_; }
...
void AgentBase::init_auth() {
  ...
  // every restart. init_auth() runs exactly once (guarded by
  // auth_initialized_), so we PRINT the generated credential ONCE to stderr
  // here — otherwise the password is unrecoverable (r5 deep_dogfood: the
  // get_basic_auth_credentials() accessor returns empty pre-run and blocks
  // during run, so a developer following docs-only cannot authenticate to
  // their own freshly-served agent). stderr, not the logger, so it surfaces
  // regardless of log level and is never captured into a structured sink.
  if (password_auto_generated) {
    get_logger().warn("basic_auth_password_autogenerated: username=\"" + auth_user_ +
    ...
    std::fprintf(stderr,
                 "\n[signalwire] Auto-generated basic-auth credentials for this run "
                 "(set SWML_BASIC_AUTH_USER / SWML_BASIC_AUTH_PASSWORD to make them "
                 "stable):\n    Basic Auth: %s:%s\n\n",
                 auth_user_.c_str(), auth_pass_.c_str());
  }
}
...
std::string AgentBase::build_webhook_url(const std::string& base_url) const {
  ...
  if (auth_initialized_ && !auth_user_.empty()) {
    ...
    if (scheme_end != std::string::npos) {
      ...
      url = scheme + signalwire::url_encode(auth_user_) + ":" + signalwire::url_encode(auth_pass_) +
            "@" + rest;
    }
  }
...
void AgentBase::setup_routes(httplib::Server& server) {
  ...
  auto wrap_post = [this](security::HttpHandler downstream) -> security::HttpHandler {
    if (!signing_key_ || signing_key_->empty()) {
      return downstream;
    }
    security::WebhookValidatorOptions opts;
    opts.trust_proxy = trust_proxy_for_signature_;
    return security::WrapWithSignatureValidation(*signing_key_, std::move(downstream), opts);
  };
  ...
  security::HttpHandler swml_get = [this](const httplib::Request& req, httplib::Response& res) {
    handle_swml_request(req, res);
  };
  security::HttpHandler swml_post =
  ...
  server.Post(swaig_path, wrap_post([this](const httplib::Request& req, httplib::Response& res) {
