...
sw_agent_t sw_agent_create_full(const char* name, const char* route, const char* host, int port) {
  try {
    auto* agent = new agent::AgentBase(name ? name : "agent", route ? route : "/",
...
void sw_agent_destroy(sw_agent_t handle) { delete static_cast<agent::AgentBase*>(handle); }
...
void sw_agent_add_section_bullets(sw_agent_t handle, const char* title, const char** bullets) {
  ...
  static_cast<agent::AgentBase*>(handle)->prompt_add_section(title, "", bullet_vec);
}
...
void sw_agent_define_tool(sw_agent_t handle, const char* name, const char* description,
  ...
  if (params_json) {
    try {
      ...
      params =
          nlohmann::json::object({{"type", "object"}, {"properties", nlohmann::json::object()}});
    }
  } else {
  ...
  agent->define_tool(
      ...
      [ctx](const nlohmann::json& args, const nlohmann::json& raw) -> swaig::FunctionResult {
        ...
        sw_function_result_t result_handle =
            ctx->handler(args_str.c_str(), raw_str.c_str(), ctx->user_data);
...
void sw_free_string(char* str) { std::free(str); }
