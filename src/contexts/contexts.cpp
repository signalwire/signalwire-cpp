...
json Step::to_json() const {
  ...
  if (step_criteria_) {
    j["step_criteria"] = *step_criteria_;
  }
  if (end_) {
    j["end"] = true;
  }
  if (skip_user_turn_) {
    j["skip_user_turn"] = true;
  }
  ...
  if (valid_steps_) {
    j["valid_steps"] = *valid_steps_;
  }
  if (valid_contexts_) {
    j["valid_contexts"] = *valid_contexts_;
  }
  ...
  json reset;
  if (reset_system_prompt_) {
    reset["system_prompt"] = *reset_system_prompt_;
  }
  if (reset_user_prompt_) {
    reset["user_prompt"] = *reset_user_prompt_;
  }
  if (reset_consolidate_) {
    reset["consolidate"] = true;
  }
  if (reset_full_reset_) {
...
Step& Context::add_step(
  ...
  if (!bullets.empty()) {
    step.add_bullets("Instructions", bullets);
  }
  if (!criteria.empty()) {
  ...
  if (!valid_steps_val.empty()) {
    step.set_valid_steps(valid_steps_val);
  }
...
Context& Context::add_enter_filler(const std::string& lang,
  ...
  if (enter_fillers_.is_null()) {
    enter_fillers_ = json::object();
  }
...
Context& Context::add_exit_filler(const std::string& lang,
                                  const std::vector<std::string>& fillers) {
  if (exit_fillers_.is_null()) {
    exit_fillers_ = json::object();
  }
...
std::optional<std::string> Context::render_system_prompt() const {
  ...
  if (system_prompt_sections_.empty()) {
    return std::nullopt;
  }
  std::string result;
...
json Context::to_json() const {
  ...
  if (user_prompt_) {
    j["user_prompt"] = *user_prompt_;
  }
  if (isolated_) {
    j["isolated"] = true;
  }
  if (post_prompt_) {
    j["post_prompt"] = *post_prompt_;
  }

  if (!enter_fillers_.is_null()) {
    j["enter_fillers"] = enter_fillers_;
  }
  if (!exit_fillers_.is_null()) {
    j["exit_fillers"] = exit_fillers_;
  }

  if (initial_step_) {
    j["initial_step"] = *initial_step_;
  }
  if (valid_contexts_) {
    j["valid_contexts"] = *valid_contexts_;
  }
