...
DataMap::DataMap(const std::string& function_name) : function_name_(function_name) {}
...
DataMap& DataMap::description(const std::string& desc) { return purpose(desc); }
...
DataMap& DataMap::webhook(const std::string& method, const std::string& url, const json& headers,
  ...
  if (!form_param.empty()) {
    wh["form_param"] = form_param;
  }
  if (input_args_as_params) {
    wh["input_args_as_params"] = true;
  }
  if (!require_args.empty()) {
    wh["require_args"] = require_args;
  }
