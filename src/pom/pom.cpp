...
Section::Section(std::optional<std::string> t, std::string b, std::vector<std::string> bs,
                 std::optional<bool> num, bool numbered_bullets)
    : title(std::move(t)),
      body(std::move(b)),
      bullets(std::move(bs)),
      numbered(num),
...
void Section::add_body(const std::string& b) { body = b; }
...
std::string Section::render_markdown(int level, const std::vector<int>& section_number) const {
  ...
  for (size_t i = 0; i < subsections.size(); ++i) {
    ...
    if (title.has_value() || !section_number.empty()) {
      ...
      bool sub_explicitly_unnumbered = sub.numbered.has_value() && !*sub.numbered;
...
std::string Section::render_xml(int indent, const std::vector<int>& section_number) const {
  ...
  if (!bullets.empty()) {
    ...
    for (size_t i = 0; i < bullets.size(); ++i) {
      if (numberedBullets) {
        parts.push_back(indent_str + "    <bullet id=\"" + std::to_string(i + 1) + "\">" +
                        bullets[i] + "</bullet>");
      } else {
        parts.push_back(indent_str + "    <bullet>" + bullets[i] + "</bullet>");
      }
    }
    parts.push_back(indent_str + "  </bullets>");
  }
...
Section PromptObjectModel::build_section(const json& d, bool is_subsection) {
  ...
  if (d.contains("numberedBullets") && !d["numberedBullets"].is_boolean()) {
    throw std::invalid_argument("'numberedBullets' must be a boolean if provided.");
  }

  bool has_body =
      d.contains("body") && d["body"].is_string() && !d["body"].get<std::string>().empty();
  bool has_bullets = d.contains("bullets") && !d["bullets"].empty();
  bool has_subsections = d.contains("subsections") && !d["subsections"].empty();
...
PromptObjectModel PromptObjectModel::from_json(const json& data) {
  if (!data.is_array()) {
    throw std::invalid_argument("Top-level POM must be a list of section dictionaries.");
  }
...
Section& PromptObjectModel::add_section(const std::string& title, const std::string& body,
                                        const std::vector<std::string>& bullets,
                                        std::optional<bool> numbered, bool numbered_bullets) {
...
void PromptObjectModel::add_pom_as_subsection(const std::string& target_title,
                                              const PromptObjectModel& pom_to_add) {
  Section* target = find_section(target_title);
  if (!target) {
    throw std::invalid_argument("No section with title '" + target_title + "' found.");
  }
  add_pom_as_subsection(*target, pom_to_add);
}

void PromptObjectModel::add_pom_as_subsection(Section& target,
                                              const PromptObjectModel& pom_to_add) {
...
std::vector<Line> split_lines(const std::string& src) {
  ...
  while (i < src.size()) {
    ...
    while (first_nonspace < content_test.size() && content_test[first_nonspace] == ' ') {
      ++first_nonspace;
    }
    ...
    for (size_t k = first_nonspace; k < content_test.size(); ++k) {
      ...
      if (c == '"' && !in_sq) {
        ...
        in_sq = !in_sq;
      } else if (c == '#' && !in_dq && !in_sq && k > 0 && content_test[k - 1] == ' ') {
        cut = k;
        break;
      }
    }
  ...
  while (idx < lines.size()) {
    ...
    for (size_t k = 0; k < trimmed_first.size(); ++k) {
      ...
      if (c == '"' && !in_sq) {
        ...
        if (k + 1 == trimmed_first.size() || trimmed_first[k + 1] == ' ') {
          colon_pos = k;
    ...
    if (is_mapping_inline) {
      ...
      std::string val_part =
          (colon_pos + 1 < trimmed_first.size()) ? trimmed_first.substr(colon_pos + 1) : "";
      ...
      if (val_part.empty()) {
        ...
        if (idx < lines.size()) {
          ...
          } else if (Next.indent == item_inline_indent && !Next.text.empty() &&
                     Next.text[0] == '-') {
...
json parse_mapping(const std::vector<Line>& lines, size_t& idx, int map_indent) {
  ...
  while (idx < lines.size()) {
    ...
    if (colon_pos == std::string::npos) {
      throw std::invalid_argument("YAML parse error: expected mapping key at line " +
                                  std::to_string(L.line_no));
    }
    std::string key = trim(L.text.substr(0, colon_pos));
    std::string val_part = (colon_pos + 1 < L.text.size()) ? L.text.substr(colon_pos + 1) : "";
    val_part = trim(val_part);
    idx += 1;
    if (val_part.empty()) {
      ...
      if (idx < lines.size()) {
        ...
        } else if (Next.indent == map_indent && !Next.text.empty() && Next.text[0] == '-') {
...
template <typename J>
void emit_mapping(std::string& out, const J& obj, int indent_level, bool inline_first) {
...
std::string yaml_dump(const json& value) { return yaml_dump_impl(value); }
...
std::string yaml_dump_ordered(const nlohmann::ordered_json& value) { return yaml_dump_impl(value); }
