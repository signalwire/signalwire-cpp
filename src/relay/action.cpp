...
Action::Action(const std::string& control_id, RelayClient* client, const std::string& call_id,
...
void Action::start_input_timers() { send_control_command("start_input_timers"); }
...
void Action::update_state(const std::string& new_state, const json& result) {
  CompletedCallback cb;
  {
    ...
    if (new_state == "finished" || new_state == "error" || new_state == "no_input" ||
        new_state == "no_match") {
...
void Action::send_control_command(const std::string& operation, const json& extra_params) {
  if (!state_->client) {
  ...
  try {
    std::string prefix =
        state_->method_prefix.empty() ? std::string("calling.play") : state_->method_prefix;
