...
std::string hmac_sha1_raw(std::string_view key, std::string_view message) {
  ...
  HMAC(EVP_sha1(), reinterpret_cast<const unsigned char*>(key.data()), static_cast<int>(key.size()),
...
ParsedUrl parse_url(std::string_view url) {
  ...
  auth_end = std::min({auth_end, path_start == std::string_view::npos ? rest.size() : path_start,
  ...
  if (!authority.empty() && authority.front() == '[') {
    ...
    if (close != std::string_view::npos) {
      ...
      if (close + 1 < authority.size() && authority[close + 1] == ':') {
        p.port.assign(authority.data() + close + 2, authority.size() - (close + 2));
      }
    } else {
