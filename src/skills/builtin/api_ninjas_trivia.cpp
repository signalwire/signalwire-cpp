...
class ApiNinjasTriviaSkill : public SkillBase {
 public:
  ...
  /// Python parity: ``get_tools`` — the SWAIG tool defs this skill contributes.
  ...
  std::vector<json> get_datamap_functions() const override {
    ...
    dm.purpose("Get trivia questions for " + tool_name_)
        ...
        .output(
            swaig::FunctionResult("Category ${array[0].category} question: ${array[0].question} "
                                  "Answer: ${array[0].answer}, be sure to give the user time to "
                                  "answer before saying the answer."));
