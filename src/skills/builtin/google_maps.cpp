 ...
 public:
  ...
  std::vector<swaig::ToolDefinition> register_tools() override {
    ...
    tools.push_back(define_tool(
        lookup_tool_, "Look up and validate an address using Google Maps",
        json::object(
            {{"type", "object"},
             {"properties",
              json::object(
        ...
        [this](const json& args, const json&) -> swaig::FunctionResult {
          ...
          return swaig::FunctionResult("Address lookup for '" + addr +
                                       "': [Would use Google Geocoding API with key " +
                                       api_key_.substr(0, 4) + "...]");
        }));

    tools.push_back(define_tool(
        route_tool_, "Compute driving route between two locations",
        json::object(
            {{"type", "object"},
             {"properties",
              json::object(
