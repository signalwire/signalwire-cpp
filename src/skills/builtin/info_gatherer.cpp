 ...
 public:
  ...
  std::vector<swaig::ToolDefinition> register_tools() override {
    ...
    tools.push_back(define_tool(
        start_name, "Start the information gathering process",
        ...
        [questions, instance_key](const json&, const json&) -> swaig::FunctionResult {
          ...
          result.update_global_data(
              json::object({{instance_key, json::object({{"question_index", 0},
                                                         {"answers", json::array()},
                                                         {"questions", questions}})}}));
          return result;
        }));
    ...
    tools.push_back(define_tool(
        submit_name, "Submit an answer to the current question",
        json::object(
            {{"type", "object"},
             {"properties",
              json::object(
        ...
        [this, questions, instance_key](const json& args,
          ...
          if (needs_confirm && !confirmed) {
            return swaig::FunctionResult("Please confirm the answer: " + answer);
          }

          answers.push_back(
              json::object({{"key", questions[q_idx].value("key_name", "")}, {"answer", answer}}));
          ...
          result.update_global_data(
              json::object({{instance_key, json::object({{"question_index", next_idx},
                                                         {"answers", answers},
                                                         {"questions", questions}})}}));
  ...
  json get_global_data() const override {
    ...
    auto questions = params_.value("questions", json::array());
    return json::object({{key, json::object({{"questions", questions},
                                             {"question_index", 0},
                                             {"answers", json::array()}})}});
  }
  ...
  std::vector<SkillPromptSection> get_prompt_sections() const override {
    ...
    return {{"Info Gatherer (" + key + ")",
             ...
             {"Start by calling start_questions to get the first question",
              "For each answer, call submit_answer", "Ask questions one at a time in order"}}};
  }
