...
class WeatherApiSkill : public SkillBase {
 public:
  ...
  /// The SWAIG tool defs this skill contributes (DataMap-backed).
  ...
  std::vector<json> get_datamap_functions() const override {
    ...
    if (temp_unit_ == "celsius") {
      output_template =
          "Weather in ${response.location.name}: ${response.current.temp_c}C "
      ...
      output_template =
          "Weather in ${response.location.name}: ${response.current.temp_f}F "
