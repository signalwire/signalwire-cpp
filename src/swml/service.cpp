  ...
  if (!env_pass.empty()) {
    ...
    if (auth_pass_.empty()) {
      throw std::runtime_error(
          "Failed to generate random password - refusing to start with weak credentials");
    }
  }
  auth_initialized_ = true;
...
std::tuple<std::string, std::string, std::string> Service::get_basic_auth_credentials_with_source()
    const {
  ...
  if (env_user && env_pass && std::string(env_user) == auth_user_ &&
      std::string(env_pass) == auth_pass_ && !auth_user_.empty() && !auth_pass_.empty()) {
    source = "environment";
...
std::pair<Service*, std::optional<json>> Service::swaig_pre_dispatch(const json&,
                                                                     const std::string&) {
...
swaig::FunctionResult Service::on_function_call(const std::string& name, const json& args,
                                                const json& raw_data) {
...
bool Service::has_tool(const std::string& name) const { return tools_.count(name) > 0; }

std::vector<std::string> Service::list_tool_names() const { return tool_order_; }

bool Service::has_function(const std::string& name) const { return tools_.count(name) > 0; }
...
std::map<std::string, swaig::ToolDefinition> Service::get_all_functions() const { return tools_; }
...
std::string Service::build_tool_registry_json() const {
  ...
  for (const auto& name : tool_order_) {
    ...
    for (const auto& raw : registered_swaig_functions_) {
      if (raw.contains("function") && raw["function"].is_string() &&
