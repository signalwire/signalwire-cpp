...
void spawn_mock_server(int port) {
  ...
  if (pid == 0) {
    ...
    execlp("python", "python", "-m", "mock_signalwire", "--host", "127.0.0.1", "--port",
           port_str.c_str(), "--log-level", "error", (char*)nullptr);
    _exit(127);
  }
...
std::string ensure_server() {
  ...
  throw std::runtime_error(
      ...
      std::to_string(kStartupTimeoutSeconds) + "s on port " + std::to_string(port) +
...
std::string active_project() { return active_project_ref(); }

std::string active_auth_header() { return active_auth_ref(); }
...
std::vector<JournalEntry> journal() {
  ...
  for (const auto& e : arr) {
    ...
    if (e.contains("query_params") && e["query_params"].is_object()) {
      for (auto it = e["query_params"].begin(); it != e["query_params"].end(); ++it) {
...
JournalEntry journal_last() {
  ...
  if (entries.empty()) {
    throw std::runtime_error("mocktest: journal is empty - SDK call did not reach the mock server");
  }
  return entries.back();
}
...
void scenario_set(const std::string& endpoint_id, int status, const json& body) {
  ...
  if (res->status != 200) {
    throw std::runtime_error("mocktest: POST /__mock__/scenarios/" + endpoint_id +
...
RestClient make_client() {
  ...
  std::string auth_header = "Basic " + signalwire::base64_encode(project + ":" + token);
