...
TEST(aiconfig_set_languages_replaces) {
  AgentBase agent;
  agent.add_language({"English", "en-US", "rachel"});
  agent.set_languages({{"Spanish", "es-ES", "isabella"}, {"French", "fr-FR", "jean"}});
  json swml = agent.render_swml();
  auto ai = find_ai_verb(swml);
...
TEST(aiconfig_set_params_multiple) {
  AgentBase agent;
  agent.set_params(
      json::object({{"temperature", 0.3}, {"top_p", 0.95}, {"barge_confidence", 0.8}}));
  json swml = agent.render_swml();
  auto ai = find_ai_verb(swml);
...
TEST(aiconfig_set_internal_fillers) {
  AgentBase agent;
  json fillers = json::object({{"en-US", json::array({"Let me think...", "One moment..."})},
