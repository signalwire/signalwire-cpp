...
TEST(function_result_add_actions) {
  FunctionResult r("test");
  r.add_actions({json::object({{"say", "hello"}}), json::object({{"stop", true}})});
  auto j = r.to_json();
...
TEST(function_result_chaining) {
  auto j = FunctionResult("chained").add_action("say", json("hi")).hangup().to_json();
...
TEST(function_result_toggle_functions) {
  FunctionResult r("test");
  r.toggle_functions(json::array({json::object({{"function", "get_weather"}, {"active", false}}),
...
TEST(function_result_tap_invalid_codec) {
  ...
  try {
    ...
    ASSERT_EQ(std::string(e.what()), "codec must be one of ['PCMU', 'PCMA']");
    checked = true;
  }
  ASSERT_TRUE(checked);
  return true;
...
TEST(function_result_record_call_invalid_direction) {
  ...
  try {
    ...
    ASSERT_EQ(std::string(e.what()), "direction must be 'speak', 'listen', or 'both'");
    checked = true;
  }
  ASSERT_TRUE(checked);
  return true;
...
TEST(function_result_execute_rpc_call_id_sibling_of_params) {
  ...
  FunctionResult r("RPC");
  r.execute_rpc("ai_message", json::object({{"role", "system"}, {"message_text", "hi"}}),
                "call-xyz", "node-7");
  auto cmd = rpc_cmd(r);
...
TEST(tool_definition_to_swaig_json) {
  ...
  td.parameters = json::object(
      {{"type", "object"},
       {"properties",
        json::object({{"city", json::object({{"type", "string"}, {"description", "City name"}})}})},
