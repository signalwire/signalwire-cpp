...
TEST(parameter_schema_byte_identical_single_string) {
  ...
  json built =
      sw_swaig::ParameterSchema{}.string("query", "Search query").required({"query"}).to_json();
       ...
       {"properties",
        json::object(
            {{"answer", json::object({{"type", "string"}, {"description", "The survey answer"}})},
             {"question_index",
              json::object({{"type", "integer"}, {"description", "Question index"}})}})},
...
TEST(parameter_schema_byte_identical_enum_property) {
  json hand = json::object(
      {{"type", "object"},
       {"properties",
        json::object({{"fmt", json::object({{"type", "string"},
...
TEST(parameter_schema_enum_from_tier1_record_format) {
  json hand = json::object(
      {{"type", "object"},
       {"properties",
        json::object({{"format", json::object({{"type", "string"},
                                               {"enum", json::array({"wav", "mp3", "mp4"})}})}})}});

  json built =
      sw_swaig::ParameterSchema{}.enum_of("format", sw_swaig::record_format_values()).to_json();
...
TEST(parameter_schema_enum_from_tier1_codec) {
  json built =
      sw_swaig::ParameterSchema{}.enum_of("codec", sw_swaig::codec_values(), "Tap codec").to_json();
...
TEST(parameter_schema_byte_identical_array_of_scalar) {
  json hand = json::object(
      {{"type", "object"},
       {"properties",
        json::object({{"tags", json::object({{"type", "array"},
                                             {"description", "Tags"},
                                             {"items", json::object({{"type", "string"}})}})}})}});

  json built = sw_swaig::ParameterSchema{}.array_of("tags", "string", "Tags").to_json();
...
TEST(parameter_schema_byte_identical_array_of_object) {
  ...
  json built = sw_swaig::ParameterSchema{}.array_of("items", item).to_json();
       ...
       {"properties",
        json::object(
            {{"name", json::object({{"type", "string"}, {"description", "Name"}})},
             ...
             {"fmt", json::object({{"type", "string"},
                                   {"description", "Format"},
                                   {"enum", json::array({"wav", "mp3", "mp4"})}})},
             {"tags", json::object({{"type", "array"},
                                    {"description", "Tags"},
                                    {"items", json::object({{"type", "string"}})}})},
             {"meta",
              json::object({{"type", "object"},
                            ...
                            {"properties",
                             json::object({{"k", json::object({{"type", "string"},
                                                               {"description", "Key"}})}})}})}})},
...
TEST(parameter_schema_no_description_omits_key) {
  ...
  json built = sw_swaig::ParameterSchema{}.string("q").to_json();
...
TEST(parameter_schema_raw_property_escape_hatch) {
  json hand = json::object(
      {{"type", "object"},
       {"properties",
        json::object({{"when", json::object({{"type", "string"},
                                             {"format", "date-time"},
                                             {"description", "Timestamp"}})},
                      {"q", json::object({{"type", "string"}, {"description", "Query"}})}})}});

  json built = sw_swaig::ParameterSchema{}
                   .property("when", json::object({{"type", "string"},
                                                   {"format", "date-time"},
                                                   {"description", "Timestamp"}}))
                   .string("q", "Query")
                   .to_json();
...
TEST(parameter_schema_require_additive_equals_required) {
  json built_plural =
      sw_swaig::ParameterSchema{}.string("a").string("b").required({"a", "b"}).to_json();
  json built_additive =
      sw_swaig::ParameterSchema{}.string("a").string("b").require("a").require("b").to_json();
...
TEST(parameter_schema_define_tool_render_and_invoke) {
  ...
  agent.define_tool("book_appointment", "Book an appointment", params,
                    [](const json& args, const json&) -> FunctionResult {
                      return FunctionResult("Booked: " + args.value("service", "") + " on " +
                                            args.value("date", ""));
                    });
  ...
  auto result = agent.on_function_call(
      "book_appointment", json::object({{"service", "haircut"}, {"date", "2026-06-05"}}),
      json::object());
  ASSERT_EQ(result.to_json()["response"].get<std::string>(), "Booked: haircut on 2026-06-05");
  return true;
}
...
TEST(parameter_schema_define_tool_matches_handwritten_render) {
  ...
  auto handler = [](const json&, const json&) -> FunctionResult { return FunctionResult("ok"); };

  json hand_params = json::object(
      {{"type", "object"},
       {"properties", json::object({{"query", json::object({{"type", "string"},
                                                            {"description", "Search query"}})}})},
       {"required", json::array({"query"})}});
  ...
  agent_built.define_tool(
      "search", "Search",
      sw_swaig::ParameterSchema{}.string("query", "Search query").required({"query"}), handler);
