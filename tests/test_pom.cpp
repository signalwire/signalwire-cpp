...
TEST(pom_section_to_json_with_subsections) {
  ...
  ASSERT_EQ(j["subsections"][0]["title"].get<std::string>(), std::string("Child"));
  ASSERT_EQ(j["subsections"][0]["body"].get<std::string>(), std::string("cb"));
  ASSERT_EQ(j["subsections"][0]["bullets"][0].get<std::string>(), std::string("cb1"));
  return true;
}
...
TEST(pom_render_markdown_simple_section_body) {
  ...
  ASSERT_EQ(pom.render_markdown(), std::string("## Hello\n\nHello world\n"));
  return true;
}
...
TEST(pom_render_markdown_section_with_bullets) {
  ...
  ASSERT_EQ(pom.render_markdown(), std::string("## Rules\n\n- x\n- y\n- z\n"));
  return true;
}
...
TEST(pom_render_markdown_subsection) {
  ...
  ASSERT_EQ(pom.render_markdown(), std::string("## Parent\n\npbody\n\n### Child\n\ncbody\n"));
  return true;
}
...
TEST(pom_render_markdown_top_level_numbered) {
  ...
  ASSERT_EQ(pom.render_markdown(), std::string("## 1. A\n\nab\n\n## 2. B\n\nbb\n"));
  return true;
}
...
TEST(pom_from_json_rejects_subsection_without_title) {
  ...
  std::string bad = "[{\"title\": \"P\", \"subsections\": [{\"body\": \"missing-title\"}]}]";
