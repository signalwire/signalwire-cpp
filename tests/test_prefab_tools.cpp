...
TEST(prefab_tools_concierge_check_availability_found) {
  ...
  auto r =
      agent.check_availability(json{{"amenity", "spa"}, {"date", "2026-07-05"}}, json::object());
...
TEST(prefab_tools_faqbot_search_faqs_ranks_matches) {
  FAQBotAgent agent;
  agent.set_faqs({json{{"question", "What are your hours?"},
                       {"answer", "9-5"},
...
TEST(prefab_tools_faqbot_search_faqs_category_boost) {
  FAQBotAgent agent;
  agent.set_faqs({json{{"question", "Billing question"},
                       {"answer", "..."},
...
TEST(prefab_tools_info_gatherer_submit_answer_advances) {
  InfoGathererAgent agent;
  json raw{{"global_data",
            {{"questions", json::array({json{{"key_name", "name"}, {"question_text", "Name?"}},
...
TEST(prefab_tools_info_gatherer_on_swml_request_dynamic_callback) {
  ...
  agent.set_question_callback([](const json&, const json&, const json&) {
    return std::vector<json>{json{{"key_name", "color"}, {"question_text", "Favourite colour?"}}};
  });
  json override = agent.on_swml_request();
...
TEST(prefab_tools_info_gatherer_on_swml_request_threads_request_to_callback) {
  ...
  agent.set_question_callback([&](const json& qp, const json& bp, const json& hd) {
    seen_qp = qp;
    seen_bp = bp;
    seen_hd = hd;
    return std::vector<json>{json{{"key_name", "k"}, {"question_text", "q?"}}};
  });
  json request = {{"query_params", {{"src", "web"}}}, {"headers", {{"x-trace", "abc"}}}};
...
TEST(prefab_tools_survey_validate_rating) {
  ...
  auto bad =
      agent.validate_response(json{{"question_id", "q1"}, {"response", "9"}}, json::object());
  ASSERT_TRUE(resp(bad).find("between 1 and 5") != std::string::npos);
  auto nan =
      agent.validate_response(json{{"question_id", "q1"}, {"response", "abc"}}, json::object());
...
TEST(prefab_tools_survey_validate_multiple_choice) {
  SurveyAgent agent;
  agent.set_questions({json{{"id", "q2"},
                            {"text", "Pick"},
                            {"type", "multiple_choice"},
                            {"options", json::array({"Red", "Blue"})}}});
  auto ok =
      agent.validate_response(json{{"question_id", "q2"}, {"response", "blue"}}, json::object());
  ASSERT_TRUE(resp(ok).find("is valid") != std::string::npos);
  auto bad =
      agent.validate_response(json{{"question_id", "q2"}, {"response", "green"}}, json::object());
...
TEST(prefab_tools_survey_validate_yes_no) {
  ...
  auto ok =
      agent.validate_response(json{{"question_id", "q3"}, {"response", "Yes"}}, json::object());
  ASSERT_TRUE(resp(ok).find("is valid") != std::string::npos);
  auto bad =
      agent.validate_response(json{{"question_id", "q3"}, {"response", "maybe"}}, json::object());
...
TEST(prefab_tools_survey_validate_unknown_question) {
  ...
  auto r =
      agent.validate_response(json{{"question_id", "nope"}, {"response", "x"}}, json::object());
