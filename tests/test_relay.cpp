...
TEST(relay_component_event_parsing) {
  ...
  j["params"] = {{"call_id", "call-001"}, {"control_id", "ctl-001"}, {"state", "finished"}};
...
TEST(relay_dial_event_parsing) {
  ...
  j["params"] = {
      {"tag", "dial-tag-001"},
      {"dial_state", "answered"},
      {"call", {{"call_id", "winner-uuid"}, {"node_id", "node-uuid"}, {"dial_winner", true}}}};
...
TEST(relay_dial_event_failed) {
  json j;
  j["event_type"] = "calling.call.dial";
  j["params"] = {{"tag", "dial-tag-002"}, {"dial_state", "failed"}};
...
TEST(relay_action_on_completed_callback_already_done) {
  ...
  bool callback_fired = false;
  action.on_completed([&](const Action&) { callback_fired = true; });
...
TEST(relay_action_on_completed_callback_exception_handled) {
  ...
  action.on_completed([](const Action&) { throw std::runtime_error("test exception"); });
