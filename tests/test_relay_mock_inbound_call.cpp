...
json state_push_frame(const std::string& call_id, const std::string& call_state,
                      const std::string& tag = "", const std::string& direction = "inbound") {
  json frame;
  frame["jsonrpc"] = "2.0";
  frame["id"] = fresh_uuid();
...
TEST(relay_mock_inbound_handler_fires_with_call_object) {
  ...
  {
    std::unique_lock<std::mutex> lock(mtx);
    cv.wait_for(lock, std::chrono::seconds(5), [&] { return seen_count.load() == 1; });
  }
...
TEST(relay_mock_answer_in_handler_journals_calling_answer) {
  ...
  spin_until([] { return !mt::journal_recv("calling.answer").empty(); }, 2000);
...
TEST(relay_mock_hangup_in_handler_journals_calling_end) {
  ...
  spin_until([] { return !mt::journal_recv("calling.end").empty(); }, 2000);
...
TEST(relay_mock_inbound_respects_max_active_calls_cap) {
  auto client = mt::make_client_with_config([](RelayConfig& cfg) { cfg.max_active_calls = 2; });
