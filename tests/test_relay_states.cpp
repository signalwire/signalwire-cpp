...
TEST(call_state_from_string_round_trips_all_known) {
  for (auto v : {CallState::Created, CallState::Ringing, CallState::Answered, CallState::Ending,
                 CallState::Ended}) {
...
TEST(message_state_from_string_round_trips_all_known) {
  for (auto v :
       {MessageState::Queued, MessageState::Initiated, MessageState::Sent, MessageState::Delivered,
        MessageState::Undelivered, MessageState::Failed, MessageState::Received}) {
...
TEST(device_to_json_byte_identical_to_hand_map) {
  ...
  Device dev{"phone", {{"to_number", "+15551112222"}, {"from_number", "+15553334444"}}};
