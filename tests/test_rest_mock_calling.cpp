...
TEST(rest_mock_calling_dial_forwards_codecs_string) {
  ...
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("dial"));
  ASSERT_EQ(j.body.at("params").value("codecs", std::string()), std::string("OPUS,G729,VP8,PCMA"));
  return true;
}

TEST(rest_mock_calling_update) {
  auto client = mocktest::make_client();
  auto body = client.calling().update({.id = "call-1", .extras = {{"state", "hold"}}});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.method, std::string("POST"));
...
TEST(rest_mock_calling_transfer) {
  ...
  auto body = client.calling().transfer(
      "call-123", {.extras = {{"destination", "+15551234567"}, {"from_number", "+15559876543"}}});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.method, std::string("POST"));
  ...
  ASSERT_EQ(j.body.at("params").value("destination", std::string()), std::string("+15551234567"));
  ASSERT_EQ(j.body.at("params").value("from_number", std::string()), std::string("+15559876543"));
  return true;
}

TEST(rest_mock_calling_disconnect) {
  auto client = mocktest::make_client();
  auto body = client.calling().disconnect("call-456", {.extras = {{"reason", "busy"}}});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.method, std::string("POST"));
...
TEST(rest_mock_calling_play_pause) {
  ...
  ASSERT_EQ(j.body.at("params").value("control_id", std::string()), std::string("ctrl-1"));
  return true;
}
...
TEST(rest_mock_calling_play_volume) {
  auto client = mocktest::make_client();
  auto body = client.calling().play_volume("call-1", {.control_id = "ctrl-1", .volume = 2.5});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
...
TEST(rest_mock_calling_record) {
  auto client = mocktest::make_client();
  auto body = client.calling().record("call-1",
                                      {.extras = {{"record", json::object({{"format", "mp3"}})}}});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
...
TEST(rest_mock_calling_record_pause) {
  ...
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("control_id", std::string()), std::string("rec-1"));
  return true;
}
...
TEST(rest_mock_calling_collect_stop) {
  ...
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("control_id", std::string()), std::string("col-1"));
  return true;
}

TEST(rest_mock_calling_collect_start_input_timers) {
  auto client = mocktest::make_client();
  auto body = client.calling().collect_start_input_timers("call-1", {.control_id = "col-1"});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
...
TEST(rest_mock_calling_detect) {
  ...
  auto body = client.calling().detect(
      "call-1", {.detect = json::object({{"type", "machine"}, {"params", json::object()}})});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("calling.detect"));
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").at("detect").value("type", std::string()), std::string("machine"));
  return true;
}
...
TEST(rest_mock_calling_detect_stop) {
  ...
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("control_id", std::string()), std::string("det-1"));
  return true;
}
...
TEST(rest_mock_calling_tap) {
  ...
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").at("tap").value("type", std::string()), std::string("audio"));
  return true;
}
...
TEST(rest_mock_calling_tap_stop) {
  ...
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("control_id", std::string()), std::string("tap-1"));
  return true;
}
...
TEST(rest_mock_calling_stream_stop) {
  ...
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("control_id", std::string()), std::string("stream-1"));
  return true;
}
...
TEST(rest_mock_calling_denoise_stop) {
  auto client = mocktest::make_client();
  auto body = client.calling().denoise_stop("call-1", {.extras = {{"control_id", "dn-1"}}});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("calling.denoise.stop"));
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("control_id", std::string()), std::string("dn-1"));
  return true;
}
...
TEST(rest_mock_calling_transcribe) {
  ...
  auto body = client.calling().transcribe(
      "call-1",
      {.extras = {{"language", "en-US"}, {"transcribe", json::object({{"engine", "google"}})}}});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("calling.transcribe"));
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("language", std::string()), std::string("en-US"));
  return true;
}
...
TEST(rest_mock_calling_transcribe_stop) {
  ...
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("calling.transcribe.stop"));
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("control_id", std::string()), std::string("tr-1"));
  return true;
}
...
TEST(rest_mock_calling_live_transcribe) {
  auto client = mocktest::make_client();
  auto body = client.calling().live_transcribe("call-1", {.extras = {{"language", "en-US"}}});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("calling.live_transcribe"));
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("language", std::string()), std::string("en-US"));
  return true;
}
...
TEST(rest_mock_calling_live_translate) {
  ...
  auto body = client.calling().live_translate(
      "call-1", {.extras = {{"source_language", "en"}, {"target_language", "es"}}});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("calling.live_translate"));
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("source_language", std::string()), std::string("en"));
  ASSERT_EQ(j.body.at("params").value("target_language", std::string()), std::string("es"));
  return true;
}
...
TEST(rest_mock_calling_send_fax_stop) {
  ...
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("calling.send_fax.stop"));
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  return true;
}
...
TEST(rest_mock_calling_receive_fax_stop) {
  ...
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("calling.receive_fax.stop"));
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  return true;
}
...
TEST(rest_mock_calling_refer) {
  auto client = mocktest::make_client();
  auto body = client.calling().refer("call-1", {.extras = {{"to", "sip:other@example.com"}}});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("calling.refer"));
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("to", std::string()), std::string("sip:other@example.com"));
  return true;
}
...
TEST(rest_mock_calling_user_event) {
  ...
  auto body = client.calling().user_event(
      "call-1",
      {.extras = {{"event_name", "my-event"}, {"payload", json::object({{"foo", "bar"}})}}});
  ASSERT_TRUE(body.is_object());
  ASSERT_TRUE(body.contains("id"));
  auto j = mocktest::journal_last();
  ASSERT_EQ(j.body.value("command", std::string()), std::string("calling.user_event"));
  ASSERT_EQ(j.body.value("id", std::string()), std::string("call-1"));
  ASSERT_EQ(j.body.at("params").value("event_name", std::string()), std::string("my-event"));
  ASSERT_EQ(j.body.at("params").at("payload").value("foo", std::string()), std::string("bar"));
  return true;
}
