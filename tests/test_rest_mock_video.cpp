...
TEST(rest_mock_video_rooms_create_stream_posts_kwargs_in_body) {
  auto client = mocktest::make_client();
  auto body = client.video().rooms.create_stream("room-1", {.url = "rtmp://example.com/live"});
  ASSERT_TRUE(body.is_object());
  auto j = mocktest::journal_last();
...
TEST(rest_mock_video_conference_tokens_reset_posts_to_reset_subpath) {
  ...
  bool empty_body = j.body.is_null() || (j.body.is_object() && j.body.empty()) ||
...
TEST(rest_mock_video_streams_update_uses_put_with_kwargs) {
  auto client = mocktest::make_client();
  auto body = client.video().streams.update("stream-2", {.url = "rtmp://example.com/new"});
  ASSERT_TRUE(body.is_object());
  auto j = mocktest::journal_last();
