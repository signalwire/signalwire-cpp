...
TEST(session_manager_validate_token_temporary_string_args) {
  ...
  ASSERT_TRUE(sm.validate_token(token, std::string("func-x"), std::string("call-77")));
  ...
  ASSERT_FALSE(
      sm.validate_token(token, std::string_view{fn_haystack}, std::string_view{"call-77"}));
  return true;
}
...
TEST(contract7_python_oracle_format_interop) {
  ...
  std::string plain =
      call_id + "." + function_name + "." + std::to_string(expiry) + "." + nonce + "." + sig;
