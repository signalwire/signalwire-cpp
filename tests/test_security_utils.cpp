...
TEST(security_utils_redact_masks_password) {
  ASSERT_EQ(RedactUrl("https://user:secret@host/path"), std::string("https://user:****@host/path"));
  return true;
}
...
TEST(security_utils_redact_url_without_credentials_unchanged) {
  ...
  ASSERT_EQ(RedactUrl("https://user@host/path"), std::string("https://user@host/path"));
  return true;
}
