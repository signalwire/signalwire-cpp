...
struct HttpsGateway {
  ...
  explicit HttpsGateway(const std::string& certs_dir) {
    ...
    srv->Post(R"(/services/[^/]+/call)", [](const httplib::Request&, httplib::Response& res) {
...
TEST(skill_mcp_verify_ssl_false_disables_cert_check) {
  ...
  if (!gw.ok) {
    std::cerr << "(skipped: could not start https gateway) ";
    return true;
  }
  ...
  json base_params = json::object(
      {{"gateway_url", gw.base()}, {"services", json::array({json::object({{"name", "svc1"}})})}});
