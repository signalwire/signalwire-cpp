...
TEST(skill_websearch_setup_with_keys) {
  auto skill = sw_skills::SkillRegistry::instance().create("web_search");
  ASSERT_TRUE(skill->setup(json::object({{"api_key", "gkey"}, {"search_engine_id", "seid"}})));
  return true;
}
...
TEST(skill_websearch_handler_works) {
  ...
  srv.Get("/customsearch/v1", [&](const httplib::Request& req, httplib::Response& res) {
    ...
    res.set_content(
        R"({"items":[{"title":"Test Result","link":"https://t/1","snippet":"hit for test query"}]})",
        "application/json");
  });
...
TEST(skill_websearch_response_prefix_wraps_success) {
  std::string resp =
      run_websearch_with_params(json::object({{"response_prefix", "AGENT_HINT: from-public-web"}}));
...
TEST(skill_websearch_response_postfix_wraps_success) {
  std::string resp =
      run_websearch_with_params(json::object({{"response_postfix", "SOURCE_NOTE: public-search"}}));
  auto b = resp.find("Test Result");
...
struct LatencyFixture {
  ...
  LatencyFixture(int num_items, int content_delay_ms) {
    srv.Get("/customsearch/v1", [this, num_items](const httplib::Request&, httplib::Response& res) {
      json items = json::array();
      for (int i = 0; i < num_items; ++i) {
        items.push_back(json::object(
            {{"title", "Result " + std::to_string(i)},
             {"link", "http://127.0.0.1:" + std::to_string(port) + "/page" + std::to_string(i)},
             {"snippet", "snippet " + std::to_string(i)}}));
      }
      res.set_content(json::object({{"items", items}}).dump(), "application/json");
    });
    ...
    srv.Get(R"(/page\d+)",
            [this, content_delay_ms](const httplib::Request&, httplib::Response& res) {
              hits.fetch_add(1);
              int slept = 0;
...
TEST(skill_websearch_snippets_only_skips_scraping) {
  ...
  auto [resp, ms] = run_latency_handler(json::object({{"snippets_only", true}}), "golang");
...
TEST(skill_websearch_overall_deadline_truncates_to_snippet_fallback) {
  ...
  auto [resp, ms] = run_latency_handler(
      json::object({{"overall_deadline", 1.0},   // budget well under the 5s content stall
...
TEST(skill_websearch_overall_deadline_sequential_falls_back) {
  ...
  auto [resp, ms] = run_latency_handler(
      json::object({{"overall_deadline", 1.0},
...
TEST(skill_websearch_per_page_timeout_honored) {
  ...
  auto [resp, ms] = run_latency_handler(
      json::object({{"per_page_timeout", 0.3},
...
TEST(skill_websearch_parallel_fast_path_scrapes_content) {
  ...
  auto [resp, ms] = run_latency_handler(
      json::object(
          {{"overall_deadline", 10.0}, {"per_page_timeout", 5.0}, {"parallel_scrape", true}}),
      "postgres");
...
TEST(skill_websearch_snippet_fallback_is_wrapped) {
  ...
  auto [resp, ms] = run_latency_handler(json::object({{"overall_deadline", 1.0},
