...
TEST(service_define_tool_dispatches_via_on_function_call) {
  ...
  bool called = false;
  json captured;
  svc.define_tool("lookup", "Look it up", json::object(),
...
TEST(service_list_tool_names_returns_registered_order) {
  Service svc;
  svc.define_tool("first", "f", json::object(),
                  [](const json&, const json&) { return FunctionResult(); });
  svc.define_tool("second", "s", json::object(),
                  [](const json&, const json&) { return FunctionResult(); });
...
TEST(service_sidecar_pattern_emits_verb_and_registers_tool) {
  ...
  svc.define_tool(
      "lookup_competitor", "Look up competitor pricing.",
      json::object({{"competitor", json::object({{"type", "string"}})}}),
      [](const json& args, const json&) -> FunctionResult {
        return FunctionResult(args["competitor"].get<std::string>() + " is $99/seat; we're $79.");
      });

  // 3. Dispatch end-to-end through on_function_call.
  auto result = svc.on_function_call("lookup_competitor", json::object({{"competitor", "ACME"}}),
                                     json::object());
  auto out = result.to_json();
...
TEST(service_build_tool_registry_json_dumps_runtime_registry) {
  ...
  svc.define_tool(ToolDefinition{
      ...
      [](const json&, const json&) -> FunctionResult { return FunctionResult("ok"); },
      false,
  });
  svc.define_tool(ToolDefinition{
      "get_weather",
