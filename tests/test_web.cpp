...
TEST(web_manual_proxy_url) {
  ...
  agent.set_auth("u", "p");
  agent.define_tool("test_tool", "Test", json::object(), [](const json&, const json&) {
...
TEST(web_proxy_from_forwarded_headers) {
  ...
  std::map<std::string, std::string> headers = {{"x-forwarded-proto", "https"},
...
TEST(web_post_prompt_url_direct_override) {
  ...
  for (const auto& verb : main) {
    if (verb.contains("ai") && verb["ai"].contains("post_prompt_url")) {
      ASSERT_EQ(verb["ai"]["post_prompt_url"].get<std::string>(), "https://custom.com/post_prompt");
      return true;
    }
  }
  ASSERT_TRUE(false);
