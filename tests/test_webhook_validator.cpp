...
TEST(webhook_scheme_b_positive_via_validate_request_with_form_params) {
  ...
  // pre-parsed map.
  ASSERT_TRUE(ValidateRequest(VEC_B_KEY, VEC_B_EXPECTED, VEC_B_URL, ParamsOrBody{vec_b_params()}));
  return true;
}
...
TEST(webhook_scheme_b_body_sha256_mismatch_rejected) {
  ...
  ASSERT_FALSE(
      ValidateWebhookSignature(VEC_C_KEY, VEC_C_EXPECTED, VEC_C_URL, R"({"event":"DIFFERENT"})"));
  return true;
}
...
TEST(webhook_missing_signing_key_throws_in_validate_request) {
  ASSERT_THROWS(ValidateRequest("", "sig", VEC_A_URL, ParamsOrBody{std::string{VEC_A_BODY}}));
  return true;
}

TEST(webhook_malformed_signature_returns_false_without_throwing) {
  // Wrong length, weird chars, base64 noise — none should throw.
  const std::vector<std::string> garbage = {"xyz", "!!!!", std::string(100, 'a'), "%%notbase64%%"};
...
TEST(webhook_validate_request_missing_signature_returns_false) {
  ASSERT_FALSE(ValidateRequest(VEC_B_KEY, "", VEC_B_URL, ParamsOrBody{vec_b_params()}));
  return true;
}
...
TEST(webhook_validate_request_with_string_arg_delegates_to_combined) {
  ...
  ASSERT_TRUE(
      ValidateRequest(VEC_A_KEY, VEC_A_EXPECTED, VEC_A_URL, ParamsOrBody{std::string{VEC_A_BODY}}));
  return true;
}
...
TEST(webhook_validator_source_uses_crypto_memcmp) {
  std::ifstream f(std::string(PROJECT_SOURCE_DIR) + "/src/security/webhook_validator.cpp");
...
TEST(webhook_scheme_b_form_concat_matches_canonical_signing_string) {
  ...
  std::string concat = std::string(url) +
                       "CallSid"
                       "CA1234567890ABCDE" +
                       "Caller"
                       "+14158675309" +
