...
std::vector<Entry> corpus() {
    return {
        ...
        {"send_sms.body",
         [] {
             return fr("").send_sms("+15551112222", "+15553334444",
        ...
        {"tap.both_full",
         [] {
             return fr("").tap("rtp://10.0.0.1:5004", "tap1", TapDirection::Both, Codec::Pcma,
                               40, "https://ex.com/tapstatus");
         }},
        ...
        {"execute_rpc.full",
         [] {
             return fr("").execute_rpc(
                 "ai_message", json::object({{"role", "system"}, {"message_text", "Hello"}}),
                 "call-abc", "node-1");
         }},
        {"rpc_dial",
         [] {
             return fr("").rpc_dial("+15551234567", "+15559876543",
        ...
        {"toggle_functions",
         [] {
             return fr("").toggle_functions(json::array(
                 {json::object({{"function", "transfer"}, {"active", false}}),
                  json::object({{"function", "lookup"}, {"active", true}})}));
         }},
        {"functions_on_speaker_timeout.true",
         [] { return fr("").enable_functions_on_timeout(); }},
        ...
        {"execute_swml.dict",
         [] {
             return fr("").execute_swml(json::object(
                 {{"version", "1.0.0"},
                  {"sections",
                   json::object({{"main", json::array({json::object(
                                              {{"answer", json::object()}})})}})}}));
         }},
