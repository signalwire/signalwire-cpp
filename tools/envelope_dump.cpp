...
// a live mock_signalwire (reusing the shared tests/mocktest.hpp harness), and for
// each case observes the RAISED typed error reduced to the shared cross-port
// artifact:
//
//     {
...
std::vector<Case> corpus() {
  ...
  cases.push_back({"envelope_200_success", false, 0, nullptr, false});

  // 404 well-formed errors[] envelope: typed error, decoded NOT_FOUND.
  cases.push_back(
      {"envelope_404_typed", true, 404,
  ...
  cases.push_back(
      {"envelope_429_retry_after", true, 429,
  ...
  cases.push_back(
      {"envelope_503_unavailable", true, 503,
  ...
  cases.push_back(
      {"envelope_500_malformed_body", true, 500, json("not-json-at-all <garbage"), false});

  // 200 whose body carries errors[]: 2xx == success, so NOTHING is raised.
  cases.push_back(
      {"envelope_200_with_error_body", true, 200,
  ...
  cases.push_back({"envelope_transport_refused", false, 0, nullptr, true});
...
int main() {
  ...
  for (const auto& c : corpus()) {
    ...
    RestClient client = [&]() -> RestClient {
      ...
      if (c.has_scenario) {
        signalwire::rest::mocktest::scenario_set(kEndpoint, c.status, c.response);
      }
      return mc;
    }();

    try {
      json body = client.fabric().addresses.list();
      (void)body;
    ...
    if (!c.transport) {
      // Count journal hits for the path (retry check: 1 == no retry). Scoped
      // to this case's client via the thread-local active scope make_client()
      // set, so a concurrent run can't cross-contaminate.
      int count = 0;
