...
std::function<void()> add_verb(const std::string& name, const json& config) {
  // This lambda THROWING is the mechanism, not a defect: outcome() above calls
  // it inside a try/catch and classifies a throw as "raised", which is the
  // corpus value this tool exists to record. Swallowing the exception here
  // would make every strict-validation case report "ok" and silently gut the
  // BEHAVIORAL-STRICT-RENDER comparison against the Python differ.
  return [name, config]() {  // NOLINT(bugprone-exception-escape)
    Service svc;
